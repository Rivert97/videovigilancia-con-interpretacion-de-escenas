...
La siguiente guía fue probada en una computadora con Windows 10, Intel Core i5-10300H, NVIDIA GeForce GTX 1050 Ti (4 GB), 8 GB RAM DDR4. Se incluyen algunos consejos para otros sistemas operativos.

El proyecto puede instalarse en modo aplicación o modo desarrollo. El modo aplicación consiste en crear y utilizar un contenedor Docker con la aplicación web y los servicios de detección de objetos, es una instalación más ligera y sirve para probar y usar la aplicación. El modo desarrollo consiste en instalar Python y todas las librerías requeridas para el desarrollo, permite usar opciones como el reentrenamiento de modelos entre otros.
...
\begin{itemize}
    ...
    \item \textbf{(Opcional) Drivers de NVIDIA:} Descargar la versión correspondiente a la tarjeta gráfica instalada. Revise la página oficial de NVIDIA\footnote{\url{https://www.nvidia.com/es-es/drivers/}}. Se puede omitir si solo se usa inferencia en CPU (más lento). En Linux, para usar la GPU es necesario instalar el `Nvidia Container Toolkit'\footnote{\url{https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html}}.
\end{itemize}

\subsection{Comandos}

1. Iniciar el servicio de \textit{Docker Desktop}. En Windows el servicio se inicia al abrir la aplicación, en Linux usualmente inicia de forma automática.

2. Crear los contenedores. Este comando descarga una imagen de Redis y construye la imagen de la aplicación. El proceso puede demorar varios minutos:
...
Bajo esta configuración, es posible visualizar el comportamiento en tiempo real de la detección de eventos. Para ello se debe detener el servicio `tracking\_service' desde la aplicación web y ejecutar el siguiente comando:
